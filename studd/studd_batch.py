...
class STUDD:
    ...
    def drift_detection_std(datastream_, model_,
        ...
        std_detector = detector(delta=delta , threshold=2)
        std_alarms = []
        ...
        plt.plot(accuracyStud, label='Student Accuracy')
        plt.plot(accuracyTeacher, label='Teacher Accuracy')
        plt.title(f"STUDD Accuracies")
        plt.xlabel("Iterations")
        plt.ylabel("Accuracy")
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.savefig(f'studd_accuracy_delta_{delta}.png')
        plt.clf()

        plt.plot(errorStud, label='STUDD Error')
        plt.title(f"STUDD Errors")
        plt.xlabel("Iterations")
        plt.ylabel("Error")
        plt.grid(True, linestyle='--', alpha=0.6)
        plt.savefig(f'studd_errors_delta_{delta}.png')
        plt.clf()
