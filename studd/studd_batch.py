...
class STUDD:
    ...
    def drift_detection_std(datastream_, model_,
        ...
        # Após o loop principal no método drift_detection_std
        plt.figure(figsize=(10, 6))
        plt.plot(range(n_train, n_train + len(errorStud)), errorStud, label="Student Model Error")
        for alarm in std_alarms:
            plt.axvline(x=alarm, color='red', linestyle='--', label=f"Drift Detected at {alarm}")
        plt.title("Error Metrics Over Time")
        plt.xlabel("Iteration")
        plt.ylabel("Error")
        plt.legend()
        plt.grid()
        plt.savefig("page_hinkley_metric.png")
        # plt.show()
